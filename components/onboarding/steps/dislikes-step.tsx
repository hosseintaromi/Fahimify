...
import DislikesPriorityInput from "../inputs/dislikes-priority-input"

interface DislikesStepProps {
  value: { item: string; priority: number }[]
  onNext: (fieldName: string, value: any) => void
  onBack: () => void
  step: number
}

export default function DislikesStep({ value, onNext, onBack }: DislikesStepProps) {
  const [selected, setSelected] = useState(value)

  return (
    <div className="space-y-8">
      <DislikesPriorityInput value={selected} onChange={setSelected} />
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <Button
          onClick={() => onNext("dislikes", selected)}
